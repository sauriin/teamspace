...
export default function WhiteboardView({ boardId = "default" }) {
    ...
    const drawStroke = (ctx, s, preview = false) => {
        ...
        } else if (s.tool === "rectangle") drawRect(ctx, s.points[0], s.points[1], s.color, s.width, preview);
        else if (s.tool === "ellipse") drawEllipse(ctx, s.points[0], s.points[1], s.color, s.width, preview);
        else if (s.tool === "arrow") drawArrow(ctx, s.points[0], s.points[1], s.color, s.width, preview);
    };
    ...
    return (
        <div ref={containerRef} className="w-full h-full relative bg-gray-100">
            ...
            <div className="fixed top-16 left-[300px] flex flex-col gap-3 p-2 rounded bg-black/50 z-20">
                <button
                    onClick={() => setTool("pen")}
                    className={`p-2 rounded ${tool === "pen" ? "bg-white text-black" : "bg-transparent text-white"}`}
                    title="Pen"
                >
                    <PenTool size={20} />
                </button>
                <button
                    ...
                    className={`p-2 rounded ${tool === "eraser" ? "bg-white text-black" : "bg-transparent text-white"}`}
                    title="Eraser"
                >
                    <Eraser size={20} />
                </button>
